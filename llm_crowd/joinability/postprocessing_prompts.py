...
class JoinPrompt:
    def __init__(self, preamble, c1sentence, c2sentence, question, questioncol1, questioncol2):
        self.preamble = preamble
        self.c1sentence = c1sentence
        self.c2sentence = c2sentence
        self.question = question
        self.questioncol1 = questioncol1
        self.questioncol2 = questioncol2
        self.lang = 'english'
    
    def get_prompt_nospaces(self, candidate1, candidate2, col1, col2, include_preamble=True):
        full_st = self.c1sentence + candidate1 + self.c2sentence + candidate2 + self.question
        full_st += col1 + self.questioncol1 + col2 + self.questioncol2
        if include_preamble:
            full_st = self.preamble + full_st
        return full_st
    
    def get_chat(self, candidate1, candidate2, col1, col2, examples, followup):
        chat = []
        include_preamble = True
        for example in examples:
            message = self.get_prompt_nospaces(example[0], example[1], example[2], example[3], include_preamble=include_preamble) + ' ' + followup
            chat.append({"role": "user", "content": message})
            include_preamble = False
            chat.append({"role": "assistant", "content": example[4]})
        message = self.get_prompt_nospaces(candidate1, candidate2, col1, col2, include_preamble=include_preamble) + ' ' + followup
        chat.append({"role": "user", "content": message})
        return chat
...
def gen_plain():
    ...
    c1sentence = 'Table 1:\n'
    c2sentence = '\n\nTable 2:\n'
    question = '\n\n Is it likely that column '
    
    questioncol1 = ' from Table 1 and column '
    questioncol2 = ' from Table 2 capture the same type of information?'
    layprompt = JoinPrompt(preamble, c1sentence, c2sentence, question, questioncol1, questioncol2)
    return layprompt
...
def gen_accountant():
    ...
    c2sentence = '\n\nSpreadsheet 2:\n'
    question = '\n\n Based on these spreadsheet samples, is it likely that column '
    
    questioncol1 = ' from Spreadsheet 1 and column '
    questioncol2 = ' from Spreadsheet 2 capture the same type of information?'
    layprompt = JoinPrompt(preamble, c1sentence, c2sentence, question, questioncol1, questioncol2)
    return layprompt
...
def gen_mleng():
    ...
    question = '\n\nDoes it make sense to add columns from this dataset to my current training dataset by joining the two datasets on column '
    questioncol1 = ' from my training dataset, and column '
    questioncol2 = ' from the dataset I found?'
    csprompt = JoinPrompt(preamble, c1sentence, c2sentence, question, questioncol1, questioncol2)
    return csprompt
...
def gen_fk():
    ...
    c2sentence = '\n\nSample of Table 2:\n'
    question = '\n\nBased on the given table samples, would it make sense to declare a foreign key dependency between Table 1 and Table 2 on column '
    questioncol1 = ' from Table 1 and column '
    questioncol2 = ' from Table 2?'
    layprompt = JoinPrompt(preamble, c1sentence, c2sentence, question, questioncol1, questioncol2)
    return layprompt
...
if __name__=='__main__':
    ...
    for i,l in enumerate(lst):
        with open('all_prompts/' + names[i] + '_joinprompt_english.pkl', 'wb') as fh:
            pickle.dump(l, fh)
    ...
    print(plain_tmp.get_prompt_nospaces(cand1, cand2, col1, col2))
    print(acc_tmp.get_prompt_nospaces(cand1, cand2, col1, col2))
    print(mleng_tmp.get_prompt_nospaces(cand1, cand2, col1, col2))
    print(lost_tmp.get_prompt_nospaces(cand1, cand2, col1, col2))
    print(fk_tmp.get_prompt_nospaces(cand1, cand2, col1, col2))